...
#ifndef KINOVA_DLL_COMMUNICATIONLAYER_H_
#define KINOVA_DLL_COMMUNICATIONLAYER_H_
...
extern "C" __declspec(dllexport) int ScanForNewDevice();

extern "C" __declspec(dllexport) int GetDevices(KinovaDevice list[MAX_KINOVA_DEVICE], int &result);

extern "C" __declspec(dllexport) int GetActiveDevice(KinovaDevice &device);
...
you cannot used the joystick anymore and the USB API(functions belows) will not be accessible.

*/
extern "C" __declspec(dllexport) int OpenRS485_Read(RS485_Message PackagesIn[50], int QuantityWanted, int &ReceivedQtyIn);

extern "C" __declspec(dllexport) int OpenRS485_Write(RS485_Message PackagesOut[50], int QtyToSend, int &QtySent);

extern "C" __declspec(dllexport) int OpenRS485_Activate(void);
